...
@section("content")
    <nav class="navbar navbar-light bg-primary">
        <div class="container-fluid">
            <span class="navbar-brand text-white">Add New Company</span>
        </div>
    </nav>
    <div class="p-2 bg-light">
        <form class="p-3 bg-white" method="POST" action="{{route("companies.store")}}">
            @csrf
            ...
            <div class="form-group d-flex mb-4">
                ...
                <div style="flex-basis: 80%;">
                    <input type="text" id="website" class="form-control" name="website" value="{{old("website")}}"/>
                    @error("website")
                        <small class="text-danger">{{$message}}</small>
                    @enderror
                </div>
